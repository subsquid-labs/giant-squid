    ...
    switch (version) {
        case 'V0': {
            return value.value.map((asset) => {
                ...
                if (asset.id.__kind === 'X1') {
                    ...
                    assert(
                        asset.id.__kind === 'Null' || asset.id.__kind === 'Here',
                        `Unsupported asset id variant ${asset.id.__kind}`
                    )
                    return {
                        id: null,
                        amount: asset.amount,
        ...
        case 'V2': {
            return value.value.map((asset) => {
                ...
                assert(
                    asset.fungibility.__kind === 'Fungible',
                    `Unsupported asset fungibility variant ${asset.fungibility.__kind}`
                )
